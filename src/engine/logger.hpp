...
class Logger {
    public:
        ...
        static void Log(const char* msg, Args... args) {
            ...
            std::tm now;
            localtime_s(&now, &t);
            printf(LOG_COLOR_LOG "[%d:%d:%d] LOG: ", now.tm_hour, now.tm_min, now.tm_sec);
            printf(msg, args...);
            ...
            file << "[" << std::setfill('0') << std::setw(2) << now.tm_hour << ":"
                        ...
                        << std::setfill('0') << std::setw(2) << now.tm_sec  << "] LOG: "
                        << message << std::endl;
            file.close();
        }
        ...
        static void Warn(const char* msg, Args... args) {
            ...
            std::tm now;
            localtime_s(&now, &t);
            printf(LOG_COLOR_WARN "[%d:%d:%d] WARN: ", now.tm_hour, now.tm_min, now.tm_sec);
            printf(msg, args...);
            printf(LOG_COLOR_RESET "\n");
            ...
            file << "[" << std::setfill('0') << std::setw(2) << now.tm_hour << ":"
                        << std::setfill('0') << std::setw(2) << now.tm_min  << ":"
                        << std::setfill('0') << std::setw(2) << now.tm_sec  << "] WARN: "
                        << message << std::endl;
            file.close();
        }
        ...
        static void Error(const char* msg, Args... args) {
            ...
            std::tm now;
            localtime_s(&now, &t);
            printf(LOG_COLOR_ERROR "[%d:%d:%d] ERROR: ", now.tm_hour, now.tm_min, now.tm_sec);
            printf(msg, args...);
            printf(LOG_COLOR_RESET "\n");
            ...
            file << "[" << std::setfill('0') << std::setw(2) << now.tm_hour << ":"
                        << std::setfill('0') << std::setw(2) << now.tm_min  << ":"
                        << std::setfill('0') << std::setw(2) << now.tm_sec  << "] ERROR: "
                        << message << std::endl;
            file.close();
        }
};
